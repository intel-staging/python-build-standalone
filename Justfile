...
release-download-distributions token commit:
  mkdir -p dist
  cargo run --release -- fetch-release-distributions --org Granulate --token {{token}} --commit {{commit}} --dest dist

# Upload release artifacts to a GitHub release.
release-upload-distributions token datetime tag:
  cargo run --release -- upload-release-distributions --org Granulate --token {{token}} --datetime {{datetime}} --tag {{tag}} --dist dist
...
release-upload-distributions-dry-run token datetime tag:
  cargo run --release -- upload-release-distributions --org Granulate --token {{token}} --datetime {{datetime}} --tag {{tag}} --dist dist -n
...
release-set-latest-release tag:
  ...
  cat << EOF > latest-release.json
  {
    "version": 1,
    "tag": "{{tag}}",
    "release_url": "https://github.com/Granulate/python-build-standalone/releases/tag/{{tag}}",
    "asset_url_prefix": "https://github.com/Granulate/python-build-standalone/releases/download/{{tag}}"
  }
  EOF
